...
Portable JSON scenarios for client implementers of Apron protocol v5. Expected
...
Each file has `format: 3`, `kind` (`replay` or `session`), `name`,
`description`, `references`, `variants`, and `expected`, and no other
top-level keys. Frames are protocol 5. Each variant has `name` and ordered
...
| record | key | sources |
|---|---|---|
| room record | `room_id` | `room_list` results (`joined` and `rooms`), `room_update` notifications (`joined` and `updated`), history `rooms` |
| message snapshot | `message_id` | `message` notifications with a `message_id`, history `entries`, embedded snapshots |
...
- Message notifications are flat snapshots:
  ...
  History `entries` are the same flat objects. A `message` notification
...
- `reply_to` (on messages) and `intro_message` (on room records) are message
  ...
  nothing.
- A room record is an element of a `room_list` result's `joined` or `rooms`,
  of a `room_update`'s `joined` or `updated`, or of a history result's
  `rooms`. Its delivery fields (`latest_log_id`, `history_log_id`,
  `member_count`, `members`) are not part of the record. A `room_update`'s
  `left` holds no records: it only removes rooms from the visible set
  (sessions only).
...
`receive` holds a `message`, `room_update`, or `reactions` notification, or a
response `{id, result}` to `history` (it has `entries`) or to `room_list` (it
has `joined` or `rooms`). Fixed response IDs label already-issued requests;
...
| `13-room-records.json` | room records from `room_list`, `room_update`, and history replace in full; embedded `intro_message`; stale room record; thread room with `parent_room_id` |
...
| `16-transient-notices.json` | `message` notifications without `message_id` install nothing, with or without `room_id`; a logged `@server` notice is a snapshot in room `@server` |
...
| `listRooms: {as, parent_room_id?}` | List the rooms not joined, or with `parent_room_id` that room's threads not joined. |
...
| `listRooms` | `room_list` | `{parent_room_id?, not_joined: true}` |
| (recovery) | `history` | `{room_id, after, before}` plus an optional positive `limit` |
| (connect) | `auth` | `{scheme: "guest"}` plus optional `name`/`client` |
| (after `auth`, cap `rooms`) | `room_list` | `{only_joined: true}`: its `joined` becomes the visible set |
...
- `rooms`: the visible rooms sorted by `room_id` in string order, each in the
  room projection above. With cap `rooms` they are the rooms joined on the
  current connection: the `joined` of the `room_list` sent after `auth`, plus
  `room_update` `joined`, minus `left`. Without it they are the rooms a
  `message` arrived in on the current connection. Room `@server` is visible
  once a message lands there on either. A client's placeholder for the default
  room before its `room_id` is known is not a room. Only server records appear;
  optimistic local echoes do not.
- `users`: one user object per `user_id`, keyed by `user_id` in string order,
  merged field by field ([PROTOCOL.md §3.3](../../../PROTOCOL.md#33-identity)) from `you`, `user` notifications,
  `members`, results' `users` (after the rest of the result), `activity`
  senders, and each `from` whose `message_id` is greater than the kept
  object's position.
...
until rooms are listed again. The record stores, floors, checkpoints, users,
and notices are kept, so each room listed again resumes from its checkpoint rather than
recovering in full ([PROTOCOL.md §4.1](../../../PROTOCOL.md#41-history), recovery from `C + 1`). (The UI may keep showing the old view meanwhile; that is not part
...
| file | covers |
|---|---|
| `core-session.json` | a server without cap `rooms`: guest auth, a first post without `room_id` whose broadcast names the default room (both confirmation orders), `activity` typing, bare `reply_to` answered by an embedded snapshot, a `@private` notice, `server` replacement, unknown notifications, rooms learned from their messages including `@server` |
...
| `membership.json` | the joined set from `room_list`, `listRooms`, `joinRoom` and `leaveRoom` settling on `{}` while `room_update` `joined` and `left` change the visible set, an `updated` thread not joined staying hidden, removal by the server |
...
| `users.json` | field-by-field user merges from `you`, `members`, `users`, `user`, and `from`; an older `from` never overwrites; a history page's `users` merge last; joins and leaves with `room_id` |
...
- Mutation results name the saved message, the created message's creation
  ...
  `room_join`, `room_leave`, and successful `command` results are `{}`.
- A `room_update` `updated` for a room not joined describes a new thread in a
  joined room; `left` names only rooms that were visible.
...
on timers and focus; the client's unit tests cover them), room members and
their counts, typing expiry, read markers, deduplication lifetime, embeds and
uploads (including `/avatar`), push, RTC, WebAuthn (see
