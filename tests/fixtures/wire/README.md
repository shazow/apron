...
Portable JSON scenarios for client implementers of Apron protocol v3. Expected
...
requests (for example `name` or `typing`) may be pipelined or reordered and
stay queued. Then:
...
- `rooms`: the visible rooms (announced on the current connection and not
  ...
  not.
- `typing`: active indicators `{room_id, from, active: true}`, sorted by
  `room_id` then `from.user_id`; `active: false` removes one.
...
| file | covers |
|---|---|
| `core-session.json` | guest auth, typing, sends in both confirmation orders, bare `reply_to` answered by an embedded snapshot, `server` replacement, unknown notifications, room records without `log_id` |
...
Not covered: `room_join`/`room_leave`, `name` response semantics,
deduplication lifetime, uploads, push, RTC, WebAuthn (see
