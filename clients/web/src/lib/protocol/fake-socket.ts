...
export class FakeSocket {
	 ...
	 * `rooms: false`; without it no listing is sent. The client sends the
	 * listing right behind `auth` (§3.2), before its result, which is answered
	 * first, as a server finishes `auth` first.
	 */
	async greet(caps: string[] = [], options: {
		...
		const listing = [...this.sent].reverse().find((frame) => frame.method === 'room_list' && (frame.params as Record<string, unknown>)?.filter === 'joined');
		if (advertised.includes('rooms') && !listing) throw new Error('client did not list its joined rooms behind auth');
		this.receive({ id: auth.id, result: { you: options.you ?? { user_id: 'guest_1', name: 'Guest' }, ...(options.token ? { token: options.token } : {}) } });
