...
1. A visitor opens the public demo, connects, authenticates as a guest, reads recent history, and posts without signup.
...
Indexes cover per-room log ranges (the primary key), the server-wide log prefix and commit time for expiration, latest-record expiry of message and reaction state, dedup expiry, and limiter expiry. A record belongs to every room its message is in just before or after it; storing a move once per room keeps each room's history a single indexed range. Benchmark the actual query plan.
...
The schema version lives in `_meta` and the `maintenance` row. A new object creates schema 2 and seeds the `general` room's creation record. An object at schema 1 (protocol v2) is upgraded once, inside one transaction charged to the one-time bootstrap reservation: identities, credentials, resource budgets, principal limits, accepted requests, the effective clock, the accounting stop, storage pressure, the account-usage snapshot, and the cleanup deadline are kept; the v2 chat tables (`room_state`, `transitions`, `messages`, `threads`) are dropped. Their content (at most about one day under rolling retention) is discarded rather than rewritten, since v2 threads have no room records. The server-wide log continues above the v2 head, and F moves to the new `general` creation record, above every discarded record, so `latest_log_id` never decreases and `history_log_id` only rises; clients whose checkpoint falls below the new floor rebuild as specified in section 9. Dropping a legacy table is a metadata operation: the native runtime measured 183 reads and 49 writes for the whole upgrade with both a 5 MB and a 25 MB legacy fixture, within the 512-row bootstrap reservation. Passkey sessions live in key-value storage and are untouched. An object with a newer schema than the code refuses to start.
