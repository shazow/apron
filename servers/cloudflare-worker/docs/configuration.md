...
| `GUEST_POSTING` | `true` lets guests post, react, and create threads under the guest quotas; default off, so guests only read until they sign in with a passkey. Announced as `ext.demo.guest_posting` |
